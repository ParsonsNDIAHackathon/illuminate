...
export interface ShippingPort extends ShippingPoint { id: string; name: string; country: string }
export interface ShippingRoute {
  ...
  segments: { from_port: string; to_port: string; waypoints: ShippingPoint[]; passages: string[] }[]
}
...
export function linkedShippingRoutes(catalog: ShippingCatalog, nodes: GNode[], edges: GEdge[], query = ''): LinkedRoute[] {
  ...
  return catalog.routes.flatMap(route => {
    ...
    const text = [supplier.name, customer.name, route.name, route.goods, ...route.segments.flatMap(s => [ports.get(s.from_port)?.name, ports.get(s.to_port)?.name, ...s.passages])].join(' ').toLowerCase()
