<template>
  <v-container fluid>
    ...
    <v-data-table :items="items" :headers="headers" density="compact" :items-per-page="50" :loading="loading">
      ...
      <template #item.artifacts="{ item }"><a v-for="a in item.artifacts" :key="a.id" :href="a.url" target="_blank" rel="noopener" class="mr-2">{{ a.kind }}</a></template>
...
const status = ref('staged'); const items = ref<any[]>([]); const loading = ref(false)
