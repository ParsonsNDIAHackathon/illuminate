...
import { linkedShippingRoutes, usesPort, type ShippingCatalog, type RouteStatus } from '../shippingMap.ts'
...
export const useShipping = defineStore('shipping', () => {
  ...
  const filtered = computed(() => scoped.value.filter(x => !status.value || x.route.status === status.value))
  ...
  return { visible, status, port, selectedId, loading, error, attempted, catalog, scoped, filtered, routes, ports, selected, select, load }
})
