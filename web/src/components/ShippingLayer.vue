<template>
  <g v-if="shipping.visible" class="shipping-layer">
    <g v-for="item in shipping.routes" :key="item.route.id" class="shipping-route" :class="[item.route.status, { selected: shipping.selectedId === item.route.id }]"
      ...
      <path :d="routePath(item.route, shipping.catalog.ports)" class="route-line" />
    </g>
    <g v-for="port in shipping.ports" :key="port.id" :transform="`translate(${(port.longitude + 180) * 3},${(90 - port.latitude) * 3})`"
       class="shipping-port" :class="{ selected: shipping.port === port.id }" role="button" tabindex="0" :aria-label="`${port.name} port, ${count(port.id)} route records`"
      ...
      <title>{{ port.name }} · {{ count(port.id) }} route records · approximate port position</title>
      <rect :x="-6 / scale" :y="-6 / scale" :width="12 / scale" :height="12 / scale" :rx="2 / scale" />
...
import { routePath, usesPort } from '../shippingMap'
...
.shipping-port rect { fill:#c7a7ff; stroke:#102b38; stroke-width:2; vector-effect:non-scaling-stroke; }
.shipping-port.selected rect,.shipping-port:focus rect { stroke:white; }
