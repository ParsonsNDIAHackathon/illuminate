<template>
  <v-card v-if="acledSelection || news.selected || graph.selected || graph.selectedEdge" class="selection-card" :class="{ collapsed }" elevation="8">
    ...
    <div v-if="!collapsed" class="body">
      <AcledInspector v-if="acledSelection" />
...
import AcledInspector from './AcledInspector.vue'
...
const props = withDefaults(defineProps<{ showAcled?: boolean }>(), { showAcled: false })
const acledSelection = computed(() => props.showAcled && acled.visible ? acled.selected : null)
...
const headIcon = computed(() => acledSelection.value ? 'mdi-map-marker-alert-outline' : news.selected ? 'mdi-newspaper-variant-outline' : graph.selected ? 'mdi-card-text-outline' : 'mdi-vector-polyline')
const headTitle = computed(() => acledSelection.value ? `${acledSelection.value.name}, ${acledSelection.value.country}` : news.selected?.title || news.selected?.url || graph.selected?.name || graph.selectedEdge?.type || '')
function close() { acled.selectedId = ''; news.selectedUrl = null; graph.select(null); graph.selectEdge(null) }
</script>
<style scoped>
