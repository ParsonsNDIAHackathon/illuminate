...
async def stage(fact: Fact, *, source: str, trust: str, model: str | None = None) -> str:
    ...
    parts = [
        ...
        "  c.trust=$trust, c.method=$method, c.model=$model, c.confidence=$conf, c.retrieved_at=$now, c.status='staged', c.source_status='staged', c.detail=$detail, c.rel_props=$rel_props_json, c.simulated=$simulated",
        "SET c += $source_meta",
