...
- When the user asks for a report, an assessment or a write-up, call generate_report. It writes an HTML document into the graph as a Report node beside its subject, so the answer is "here is what it found, it is on the Reports tab", not a wall of prose in the chat. The default is a risk assessment of the focused program. Asking again for the same kind and subject regenerates that one report from current data; that is the intended way to refresh it.
